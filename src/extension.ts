...
export function activate(context: vscode.ExtensionContext) {
	...
	context.subscriptions.push(vscode.commands.registerCommand('vscode.conemu', (uri?: vscode.Uri) => {
		if (!isCompatiblePlatform || !checkConfiguration()) {
			return
		}
		...
		if (uri && uri.scheme && uri.scheme !== "untitled") {
			...
			// writeOutput(`vscode.conemu(uri: ${uri}, target: ${target})`)
			...
			// writeOutput(`vscode.conemu(document.uri: ${vscode.window.activeTextEditor.document.uri}, target: ${target})`)
		} else if (vscode.workspace.rootPath) {
			target = vscode.workspace.rootPath
			// writeOutput(`vscode.conemu(rootPath: ${target})`)
		}
